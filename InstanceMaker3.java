...
public class InstanceMaker3{
			...
			if (i % 2 == 0) {
				for (int j = 0; j < col; j += 2) {
					...
					if (i != 0) {
						...
						stuff.add(graphVertices[row - 1][j]);
					}
					if (i != row - 1) {
						stuff.add(graphVertices[i + 1][j]);
					} else {
						stuff.add(graphVertices[0][j]);
					}
			...
			if (j % 2 == 0) {
				for (int i = 1; i < row; i += 2) {
					...
					if (j != 0) {
						...
						stuff.add(graphVertices[i][col - 1]);
					}
					if (j != col - 1) {
						stuff.add(graphVertices[i][j + 1]);
					} else {
						stuff.add(graphVertices[i][0]);
					}
					dict.put(graphVertices[i][j], stuff);
				}
			} else {
	...
	public static void main(String[] args) {
		File file = new File("CYCLECONNECTEDALLSIDES.in");
    	try {
        writer = new PrintWriter(file);
        InstanceMaker3.initializer(22, 22);
        InstanceMaker3.matrixfy();
        writer.close();       
    	}
